...
def calcula():
    ...
    leitura_faixas = [
        valores_padrao[x].get(str(selecionado[x].get())) for x in range(modo)
    ]
    ...
    if modo != 5:
        final = (
            str(leitura_faixas)
            + "  =  "
            + str(eng((leitura_faixas[0] * 10 + leitura_faixas[1]) * leitura_faixas[2]))
            + chr(937)
            + "  "
            + chr(177)
            + str(leitura_faixas[3])
            + "%"
        )
    else:
        final = (
...
def selec_faixa():
    ...
    if opcao.get() == 2:
        ...
        c = tk.ttk.Combobox(
            janela, values=list(valores_padrao[4]), textvariable=selecionado[4]
        )
...
def desenha_botoes():
    ...
    for y in range(4):
        ...
        c = tk.ttk.Combobox(
            janela, values=list(valores_padrao[y]), textvariable=selecionado[y]
        )
        c.configure(width=10)
        c.grid(sticky="E", row=ii, column=i, columnspan=2)
